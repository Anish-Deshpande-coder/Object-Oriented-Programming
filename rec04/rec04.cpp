...
public:
    ...
    void movesInWith(Person& newRoommate) {
        ...
        if (newRoommate.roomie != nullptr) {
            cout << newRoommate.name << " already has a roommate!" << endl;
            return;
        }
        if (&newRoommate == this) {
            cout << name << " cannot move in with themselves!" << endl;
            return;
        }
        roomie = &newRoommate;
...
private:
    Person* roomie;  // Pointer to roommate
